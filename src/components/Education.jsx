...
export default function Education() {
  return (
    <section className="relative bg-gray-950 py-28 px-6 md:px-16 overflow-hidden" id="education">
      ...
      <div className="max-w-6xl mx-auto relative">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 via-blue-400 to-purple-400 ">
            <span className="drop-shadow-[0_0_8px_rgba(34,211,238,0.6)]">Academic Background</span>
          </h2>
          ...
          <div className="space-y-12">
            {educationData.map((edu, index) => (
              ...
              >
                ...
                <div className="absolute left-8 top-0 h-full w-0.5 bg-gradient-to-b from-cyan-500/20 via-purple-500/20 to-transparent origin-top group-first:origin-center group-last:h-1/2"></div>
                <motion.div 
                ...
                <div className="relative pl-20 pb-10">
                  ...
                  <motion.div 
                    className={`bg-gray-900/80 p-8 rounded-xl border border-${edu.color}-400/20 shadow-sm hover:shadow-lg transition-all duration-300 group-hover:border-${edu.color}-400/50 backdrop-blur-sm relative overflow-hidden`}
                    whileHover={{ y: -5 }}
                  >
                    ...
                    {/* Glow effect */}
                    <div className={`absolute -inset-1 bg-gradient-to-r from-${edu.color}-500/10 to-purple-500/10 rounded-xl blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-500`}></div>

                    <div className="flex items-start gap-6 relative z-10">
                      {/* Institution icon */}
                      <div className={`flex items-center justify-center p-4 rounded-lg bg-${edu.color}-900/20 border border-${edu.color}-400/20 text-${edu.color}-400 shadow-inner group-hover:shadow-${edu.color}-400/20 transition-all duration-300`}>
                        {edu.icon || <AcademicCapIcon className="h-6 w-6" />}
                      </div>

                      <div className="flex-1">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                          <h3 className="text-2xl font-bold text-white">{edu.institution}</h3>
                          <span className={`inline-flex items-center gap-1 px-3 py-1 bg-${edu.color}-900/30 text-${edu.color}-300 text-sm font-medium rounded-full border border-${edu.color}-400/20 group-hover:border-${edu.color}-400/50 transition-all duration-300`}>
                            {edu.location}
                          </span>
                        </div>
                        
                        <h4 className={`mt-2 text-lg font-semibold text-${edu.color}-400`}>{edu.degree}</h4>
                        
                        {edu.specialization && (
                          <p className={`mt-2 text-gray-300 font-medium group-hover:text-${edu.color}-300 transition-colors`}>
                            <LightBulbIcon className={`h-4 w-4 inline-block mr-1 text-${edu.color}-400`} />
                            {edu.specialization}
                          </p>
                        )}
                        ...
                        <ul className="mt-4 space-y-3">
                          {edu.highlights.map((point, i) => (
                            <motion.li 
                              ...
                              viewport={{ once: true }}
                            >
                              <span className={`inline-block w-1.5 h-1.5 bg-${edu.color}-500 rounded-full mt-2 mr-2 group-hover:bg-${edu.color}-400 transition-colors`}></span>
                              {point}
                            </motion.li>
                          ))}
                        </ul>
